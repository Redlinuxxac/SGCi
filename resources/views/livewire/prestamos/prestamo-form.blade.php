<form wire:submit.prevent="save">
    <h2 class="text-xl font-semibold mb-4">{{ $prestamo ? __('prestamos.edit_title') : __('prestamos.create_new_title') }}</h2>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <!-- Socio -->
        <div>
            <label for="socio_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('prestamos.table_header_member') }}</label>
            <select wire:model="socio_id" id="socio_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                <option value="">{{ __('prestamos.select_member') }}</option>
        ...
        <!-- Monto -->
        <div>
            <label for="monto" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('prestamos.table_header_amount') }}</label>
        ...
        <!-- Plazo en Meses -->
        <div>
            <label for="plazo_meses" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('prestamos.term_months') }}</label>
        ...
        <!-- Fecha de Desembolso -->
        <div>
            <label for="fecha_desembolso" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('prestamos.disbursement_date') }}</label>
        ...
        <div>
            ...
            <select wire:model="estado" id="estado" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                ...
                <option value="aprobado">{{ __('app.aprobado') }}</option>
                <option value="desembolsado">{{ __('app.desembolsado') }}</option>
                <option value="cancelado">{{ __('app.cancelado') }}</option>
                <option value="pagado">{{ __('app.pagado') }}</option>
            </select>
        ...
        <div class="md:col-span-2">
            <label for="observaciones" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('prestamos.observations') }}</label>
