<div>
    <h2 class="text-xl font-semibold mb-4">{{ $asiento ? __('contabilidad.view_entry_title') : __('contabilidad.create_new_entry_title') }}</h2>

    <form wire:submit.prevent="save">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
                <label for="fecha" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.Fecha') }}</label>
            ...
            <div class="md:col-span-2">
                <label for="descripcion" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.Descripción') }}</label>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        <th class="px-4 py-2 text-left">{{ __('app.Cuenta') }}</th>
                        <th class="px-4 py-2 text-left">{{ __('app.Debe') }}</th>
                        <th class="px-4 py-2 text-left">{{ __('app.Haber') }}</th>
                        <th class="px-4 py-2 text-left">{{ __('app.Descripción') }}</th>
                        @if(!$isReadOnly)
                        <th></th>
                        @endif
                    </tr>
                        ...
                        <tr wire:key="movimiento-{{ $index }}">
                            <td>
                                <select wire:model="movimientos.{{ $index }}.cuenta_contable_id" @if($isReadOnly) disabled @endif class="w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:border-gray-600">
                                    <option value="">{{ __('contabilidad.select_account') }}</option>
                ...
                <tfoot>
                    <tr>
                        <td colspan="5" class="text-right pt-2">
                            <button type="button" wire:click="addMovimiento" class="btn btn-secondary">{{ __('contabilidad.add_row') }}</button>
                        </td>
                    </tr>
                </tfoot>
                @endif
        ...
        @error('balance') <div class="mt-2 text-red-500 text-sm">{{ __('contabilidad.balance_error_debit_credit_mismatch') }}</div> @enderror
        @error('balance') <div class="mt-2 text-red-500 text-sm">{{ __('contabilidad.balance_error_zero_value') }}</div> @enderror
        ...
        <div class="mt-6 flex justify-end space-x-4">
            ...
            <button type="submit" class="btn btn-primary">
                {{ __('contabilidad.save_entry_button') }}
            </button>
            @endif
        </div>
    </form>
